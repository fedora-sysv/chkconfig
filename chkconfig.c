...
static void usage(char *name) {
    fprintf(stderr,
            _("%s version %s - Copyright (C) 1997-2000 Red Hat, Inc.\n"),
            progname, VERSION);
...
static void display_list_systemd_note() {
    if (access(SYSTEMD_SERVICE_PATH, F_OK) >= 0 && systemdIsInit()) {
        fprintf(stderr, _("\nNote: This output shows SysV services only and "
                          "does not include native\n"
                          "      systemd services. SysV configuration data "
                          "might be overridden by native\n"
                          "      systemd configuration.\n\n"));

        fprintf(stderr,
                _("      If you want to list systemd services use 'systemctl "
                  "list-unit-files'.\n"
...
static void readServiceError(int rc, char *name) {
    if (rc == 2) {
        fprintf(stderr,
                _("service %s supports chkconfig, but is not referenced in any "
                  "runlevel (run 'chkconfig --add %s')\n"),
                name, name);
    } else if (rc == 1) {
...
static void checkDeps(struct service *s, struct dep *deps, struct service *serv,
                      int start) {
    int j, k;

    for (j = 0; deps[j].name; j++) {
        if (!strcmp(deps[j].name, serv->name) &&
            isSimilarlyConfigured(*s, *serv)) {
            if (start)
        ...
        if (serv->provides) {
            for (k = 0; serv->provides[k]; k++) {
                if (!strcmp(deps[j].name, serv->provides[k]) &&
                    isSimilarlyConfigured(*s, *serv)) {
                    if (start)
                        s->sPriority = laterThan(s->sPriority, serv->sPriority);
                    else
                        s->kPriority =
                            earlierThan(s->kPriority, serv->kPriority);
                    deps[j].handled = 1;
                }
            }
        }
...
static int frobOneDependencies(struct service *s, struct service *servs,
    ...
    if (depfail) {
        // go through the missing dependecies and try to find matching systemd
        // unit
        checkSystemdDependencies(s);
        if (s->startDeps) {
            for (i = 0; s->startDeps[i].name; i++) {
                if (!s->startDeps[i].handled &&
                    strcmp(s->startDeps[i].name, "$local_fs"))
                    return -1;
            }
        }
        if (s->stopDeps) {
            for (i = 0; s->stopDeps[i].name; i++) {
                if (!s->stopDeps[i].handled &&
                    strcmp(s->stopDeps[i].name, "$local_fs"))
                    return -1;
            }
        }
    }
